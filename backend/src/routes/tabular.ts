...
tabularRouter.patch("/:reviewId", requireAuth, async (req, res) => {
    ...
    if (sharedWithUpdate !== undefined) {
        ...
        if (missingSharedUsers.length > 0) {
            return void res.status(400).json({
                detail: `${missingSharedUsers[0]} does not belong to a Courier user.`,
            });
        }
...
function buildTabularMessages(
    ...
    const systemContent = `You are Courier, an AI legal assistant. You are helping with the tabular review titled "${reviewTitle}".
